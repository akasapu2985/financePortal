...
- **[Kaladin] FinBERT headline sentiment scoring**: Added `pipeline/src/intelligence/finbert.py` for lazy-loaded `ProsusAI/finbert` CPU scoring with batch inference, neutral-threshold gating, and article annotation/filtering helpers that populate `finbert_score`/`finbert_label` metadata before LLM forwarding. Added mocked unit coverage in `pipeline/tests/test_finbert.py` and registered `transformers` + `torch` in `backend/pyproject.toml` for issue #83.
- **[Kaladin] Semantic headline deduplication**: Added `pipeline/src/intelligence/dedup.py` for lazy-loaded `sentence-transformers` headline embeddings, seven-day cosine-similarity deduping ahead of FinBERT, and article filtering helpers that attach `headline_embedding` metadata before LLM forwarding. Added mocked unit coverage in `pipeline/tests/test_dedup.py` and registered `sentence-transformers` + `numpy` in `backend/pyproject.toml` for issue #84.
